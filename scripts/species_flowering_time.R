species_flowering_time <- cache_csv("data_cache/species_flowering_time.csv", function() {
  ...
  #read in additional data from Ruby Stephens' data collection
  ...
  #clean any iffy characters from flowering data
  flowering$canonicalName <- str_trim(flowering$canonicalName)
  
  #remove any subspecies or variants in species_flowering_time trait taxa so that only species
  #level identifications remain
  flowering$canonicalName <-
    gsub("\\s(?:subsp|var|f|sect)\\.\\s.+$", "", flowering$canonicalName, perl = TRUE)
  
  #define functions - convert YN in value column to more usable binary values
  ...
  #this function sums all the 1s in the binary together to give a total number of months
  #in the year flowering e.g. 011000101 = 4
